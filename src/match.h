...
#define PATTERN_LIST(...)                                                      \
  ((patternlist_t) {                                                            \
    sizeof((pattern_t *[]){__VA_ARGS__}) / sizeof(pattern_t *),                \
        (pattern_t *[]) {                                                      \
      __VA_ARGS__                                                              \
    }                                                                          \
  })
...
#define NARY_AS(capture, op, ...)                                              \
  (&(pattern_t) {                                                               \
    EXPR_NARY, &capture, .nary = { op, .args = PATTERN_LIST(__VA_ARGS__) }     \
  })
...
#define NARY_FIXED_AS(capture, op, ...)                                        \
  (&(pattern_t) {                                                               \
    EXPR_NARY_FIXED, &capture, .nary = {                                       \
      op,                                                                      \
      .args = PATTERN_LIST(__VA_ARGS__)                                        \
    }                                                                          \
  })
